...
def insertWeekly(conn, df):
	"""	插入每周因子残差数据表
	"""
	dbutil.insertDelta(conn, df, 'FactorTradingDayTopBottomDelta')

def insertMonthly(conn, df):
	"""	插入每月因子残差数据表
	"""
	dbutil.insertDelta(conn, df, 'FactorMonthlyTopBottomDelta')
...
if __name__ == '__main__':
	"""用法： python deltadb.py 'd|w|m' '20140101' '20141231' ['filepath']
	"""
	dtype = 'd'
	...
	if len(filepath) == 0:
		filepath='D:/workspace/python/residual/delta/'
